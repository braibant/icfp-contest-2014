...
and untype_pattern pat =
  ...
  match pat with
    ...
    | Tpat_tuple list ->
        lower_pat_tuple loc (List.map untype_pattern list)
    | Tpat_construct (lid, _, args, explicit_arity) ->
        Ppat_construct (lid,
          (match args with
            ...
            | args -> Some
                  { ppat_desc = lower_pat_tuple loc (List.map untype_pattern args);
  ...
  let desc =
    ...
    | Texp_try (exp, list) ->
        Pexp_try (untype_expression exp,
          List.map (fun (pat, exp) ->
              untype_pattern pat, untype_expression exp) list)
    | Texp_tuple list ->
        lower_exp_tuple loc (List.map untype_expression list)
    | Texp_construct (lid, _, args, explicit_arity) ->
        Pexp_construct (lid,
          ...
          | args -> Some
            { pexp_desc = lower_exp_tuple loc (List.map untype_expression args);
...
let untype_implementation (structure, _coercion) =
  untype_structure structure
